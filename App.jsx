...
function getBoxContentLines(content, sourceKey = "") {
  if (Array.isArray(content)) {
    return content.flatMap((entry, index) => {
      if (typeof entry === "string") {
        ...
        if (!text) return [];
        if (sourceKey === "oxford") {
          const nextEntry = content[index + 1];
          const nextText = typeof nextEntry === "string" ? nextEntry.trim() : "";
          const explicitHeading = isBoxHeadingLine(text, sourceKey);
          const definitionHeading =
            !explicitHeading &&
            nextText.length > 80 &&
            text.length <= 70 &&
            /^[A-Z][-A-Za-z0-9’'()/., ]{1,70}$/.test(text) &&
            !/[.;]$/.test(text);
          return [{ text, kind: explicitHeading || definitionHeading ? "heading" : "item" }];
        }
        return [{ text, kind: isBoxHeadingLine(text, sourceKey) ? "heading" : "item" }];
      }
      ...
      if (entry.type === "section") {
        const lines = [];
        if (entry.heading) lines.push({ text: String(entry.heading).trim(), kind: "heading" });
        if (Array.isArray(entry.items)) {
          entry.items.forEach(item => {
            const text = String(item || "").trim();
            if (text) lines.push({ text, kind: "item" });
          });
        }
        return lines;
      }

      if (entry.type === "text" && Array.isArray(entry.lines)) {
        return entry.lines
          .map(line => String(line || "").trim())
          .filter(Boolean)
          .map(text => ({ text, kind: isBoxHeadingLine(text, sourceKey) ? "heading" : "item" }));
      }

      const text = String(entry.text || entry.heading || entry.title || "").trim();
      return text ? [{ text, kind: isBoxHeadingLine(text, sourceKey) ? "heading" : "item" }] : [];
    });
  }
  ...
  if (explicitLines.length > 1) {
    return explicitLines.map(line => ({ text: line, kind: isBoxHeadingLine(line, sourceKey) ? "heading" : "item" }));
  }
  ...
  return text
    ...
    .map(line => line.trim())
    .filter(Boolean)
    .map(line => ({ text: line, kind: isBoxHeadingLine(line, sourceKey) ? "heading" : "item" }));
}
                ...
                <>
                  ...
                  <div className="pinakakia-content-text">
                    {contentLines.map((line, index) => {
                      return (
                        <div
                          className={`pinakakia-content-line ${line.kind === "heading" ? "heading" : "item"}`}
